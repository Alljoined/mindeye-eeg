...
class BrainDiffusionPrior(DiffusionPrior):
    ...
    - Option to include the eeg2clip model and pass voxels into forward method
    ...
    def forward(
        ...
        **kwargs
        ...
        if exists(voxel):
            assert exists(
                self.eeg2clip
            ), "eeg2clip must be trained if you wish to pass in voxels"
            assert not exists(text_embed), "cannot pass in both text and voxels"
            if self.eeg2clip.use_projector:
                clip_voxels_mse, clip_voxels = self.eeg2clip(voxel)
                text_embed = clip_voxels_mse
            else:
                clip_voxels = self.eeg2clip(voxel)
                text_embed = clip_voxels_mse = clip_voxels
            # text_embed = self.eeg2clip(voxel)
