...
def Image_to_torch(x):
    try:
        x = (transforms.ToTensor()(x)[:3].unsqueeze(0) - 0.5) / 0.5
    except:
        x = (transforms.ToTensor()(x[0])[:3].unsqueeze(0) - 0.5) / 0.5
    return x
...
def topk(similarities, labels, k=5):
    ...
    topsum = 0
    for i in range(k):
        topsum += torch.sum(torch.argsort(similarities, axis=1)[:, -(i + 1)] == labels) / len(
            labels
        )
    return topsum
...
def soft_cont_loss(student_preds, teacher_preds, teacher_aug_preds, temp=0.125, distributed=True):
    ...
    loss2 = (
        -(student_teacher_aug_t.log_softmax(-1) * teacher_teacher_aug_t.softmax(-1)).sum(-1).mean()
    )
...
def mixco(voxels, beta=0.15, s_thresh=0.5):
    ...
    voxels[select] = voxels[select] * betas[select].reshape(*betas_shape) + voxels_shuffle[
        select
...
def mixco_clip_target(clip_target, perm, select, betas):
    ...
    clip_target[select] = clip_target[select] * betas[select].reshape(-1, 1) + clip_target_shuffle[
        select
    ] * (1 - betas[select]).reshape(-1, 1)
    return clip_target
    ...
    if perm is not None and betas is not None and select is not None:
        ...
        if bidirectional:
            loss2 = F.cross_entropy(
                brain_clip.T, torch.arange(brain_clip.shape[0]).to(brain_clip.device)
            )
            loss = (loss + loss2) / 2
        return loss
...
def get_huggingface_urls(commit="main", subj=1):
    ...
    train_url = (
        base_url + commit + f"/webdataset_avg_split/train/train_subj0{subj}_" + "{0..17}.tar"
    )
...
def query_laion(
    ...
    try:
        ...
        try:
            ...
            result = client.query(
                text=text, embedding_input=emb.tolist() if emb is not None else None
            )
            res_length = len(result)
            if verbose:
                print(result)
    ...
    for res in result:
        ...
        try:
            ...
            tries += 1
            if verbose:
                print("retrieved_images", retrieved_images.shape)
        except:
            time.sleep(np.random.rand())
    if verbose:
        print("final retrieved_images", retrieved_images.shape)
    return retrieved_images
...
def reconstruction(
            ...
            if recons_per_sample > 0:
                if not img_variations:
                    ...
                    brain_clip_embeddings = diffusion_prior.p_sample_loop(
                        ...
                        text_cond=dict(text_embed=brain_clip_embeddings0),
                        cond_scale=1.0,
                        timesteps=1000,  # 1000 timesteps used from nousr pretraining
                        generator=generator,
                    )
    ...
    if retrieve:
        image_retrieved = query_laion(
            emb=cls_embeddings.flatten(),
            groundtruth=None,
            num=num_retrieved,
    ...
    if retrieve == False:
        ...
        for ii, im in enumerate(retrieved_clips):
            ...
            if verbose:
                print(v2c_reference_out.shape, currecon.shape)
            cursim = batchwise_cosine_similarity(v2c_reference_out, currecon)
            sims.append(cursim.item())
        if verbose:
            print(sims)
    ...
    for ii, i in enumerate(
        range(
            num_xaxis_subplots - laion_samples - recons_per_sample,
            num_xaxis_subplots - laion_samples,
        )
    ):
...
def dynamic_cfg(noise_pred_uncond, noise_pred_text, guidance_scale):
    ...
    dt = (
        noise_pred_uncond + (noise_pred_text - noise_pred_uncond) * dynamic_thresholding_mimic_scale
    )
